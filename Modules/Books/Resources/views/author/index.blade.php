...
@section('content')
    ...
    <div class="col-lg-10 col-lg-offset-1">
        <h5>
            <i class="fa fa-users"></i> لیست نویسندگان
        ...
        <div class="table-responsive">
            <div class="fixed-table-toolbar" style="padding: 5px;margin-bottom: 3px; border-radius: 2px ;border: 1px solid #dee2e6">
                <form method="GET" action="{{route('author.list')}}">
                    ...
                    <table class="toolbar_search">
                        ...
                        <tr>
                            ...
                            <th scope="col"><input class="form-control" placeholder="ملیت" type="text" name="s_nationality" value="{{ request()->get('s_nationality') }}"> </th>
                            <th scope="col">
            ...
            <table class="table table-bordered table-striped list-table">
                ...
                @foreach ($authors as $key=>$author)
                    <tr>
                        ...
                        <th scope="row">{{ $k }}</th>
                        <td >{{ $author->name }}</td>
                        <td><img src="{{ asset('/images/authors/' . $author->avatar) }}" width="100" /></td>
                        <td>{{ $author->nationality }}</td>
                            <?php $v = new Verta($author->created_at); ?>
                        ...
                        <td>
                            <div class="row">
                                <div class="col-12" style="margin-bottom: 3px;">
                                    <a href="{{ route('author.edit', $author->id) }}?page={{request()->get('page',1)}}&s_status={{request()->get('s_status')}}&s_user_id={{request()->get('s_user_id')}}&s_name={{request()->get('s_name')}}&s_nationality={{request()->get('s_nationality')}}" class="btn btn-info pull-right btn-width">ویرایش</a>
            ...
            {{ $authors->appends(['s_status' => request()->get('s_status') , 's_nationality' => request()->get('s_nationality') , 's_name' => request()->get('s_name') , 's_user_id' => request()->get('s_user_id')])->links() }}
        </div>

        <a href="{{ route('author.create') }}?page={{request()->get('page',1)}}&s_status={{request()->get('s_status')}}&s_user_id={{request()->get('s_user_id')}}&s_name={{request()->get('s_name')}}&s_nationality={{request()->get('s_nationality')}}" class="btn btn-success">نویسنده جدید</a>
