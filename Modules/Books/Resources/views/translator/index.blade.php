...
@section('content')
    ...
    <div class="col-lg-10 col-lg-offset-1">
        <h5>
            <i class="fa fa-language"></i> لیست مترجمین
        ...
        <div class="table-responsive">
            <div class="fixed-table-toolbar" style="padding: 5px;margin-bottom: 3px; border-radius: 2px ;border: 1px solid #dee2e6">
                <form method="GET" action="{{route('translator.list')}}">
                    ...
                    <tr>
                        ...
                        <td >{{ $translator->name }}</td>
                            @if($translator->avatar != '' && file_exists(public_path().'/images/translators/'.$translator->avatar))
                                <td><img src="{{ asset('images/translators/' . $translator->avatar) }}" width="100" /></td>
                            @else
                                <td><img src="{{ asset('images/translators/avatar.png') }}" width="100" /></td>
                            @endif
                            <?php $v = new Verta($translator->created_at); ?>
                        <td>{{ $v->format(' H:i:s Y/n/j') }}</td>
                        <td>{{ $translator->user->name }}</td>
                        <td>
                            @if ($translator->status === "E")
                        ...
                        <td>
                            <div class="row">
                                <div class="col-12" style="margin-bottom: 3px;">
                                    <a href="{{ route('translator.edit', $translator->id) }}?page={{request()->get('page',1)}}&s_status={{request()->get('s_status')}}&s_user_id={{request()->get('s_user_id')}}&s_name={{request()->get('s_name')}}&s_nationality={{request()->get('s_nationality')}}" class="btn btn-info pull-right btn-width">ویرایش</a>
                            ...
                            <div class="row">
                                ...
                                {!! Form::open(['method' => 'DELETE', 'route' => ['translator.destroy', $translator->id] ]) !!}
            ...
            {{ $translators->appends(['s_status' => request()->get('s_status')  , 's_name' => request()->get('s_name') , 's_user_id' => request()->get('s_user_id')])->links() }}
        </div>

        <a href="{{ route('translator.create') }}?page={{request()->get('page',1)}}&s_status={{request()->get('s_status')}}&s_user_id={{request()->get('s_user_id')}}&s_name={{request()->get('s_name')}}" class="btn btn-success">مترجم جدید</a>
