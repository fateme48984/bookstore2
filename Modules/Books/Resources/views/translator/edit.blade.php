...
@section('content')
    ...
    <div class='col-lg-7 col-lg-offset-4'>
        ...
        <h4><i class='fa fa-language'></i> ویرایش</h4>
        <hr>
        ...
        @endif
            ...
            @endif
            {{ Form::model($translator, array('route' => array('translator.update', $translator->id), 'method' => 'PUT', 'files'=>true)) }}{{-- Form model binding to automatically populate our fields with user data --}}
            <div class="row" >
                ...
                <div class="col-lg-5">
                        <div style="margin-top: 8px;margin-bottom: 5px;">
                            @if($translator->avatar != '' && file_exists(public_path().'/images/translators/'.$translator->avatar))
                                <img src="{{ asset('images/translators/' . $translator->avatar) }}" style="width: 100%"  />
                            @else
                                <img src="{{ asset('images/translators/avatar.png') }}" style="width: 100%"  />
                            @endif
                        </div>
            ...
            <div class="form-group">
                {{ Form::label('torder', 'ترتیب') }}
                {{ Form::text('torder', null, array('class' => 'form-control')) }}
            </div>

            <div class="form-group">
                {{ Form::label('status', 'وضعیت') }}
                {{ Form::select('status', array('E' => 'فعال', 'D' => 'غیرفعال'),$translator->status, array('class' => 'form-control')) }}
            </div>
            ...
            <a class="btn btn-secondary" href="{{ route('translator.list') }}?page={{request()->get('page',1)}}&s_status={{request()->get('s_status')}}&s_user_id={{request()->get('s_user_id')}}&s_name={{request()->get('s_name')}}" role="button">بازگشت</a>
            {{ Form::close() }}
    </div>
