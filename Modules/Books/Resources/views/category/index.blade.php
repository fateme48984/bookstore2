...
@section('content')
    ...
    <div class="col-lg-10 col-lg-offset-1">
        <h5>
            <i class="fa fa-list"></i> لیست موضوعات
        ...
        <div class="table-responsive">
            ...
            <table class="table table-bordered table-striped list-table">
                ...
                @foreach ($categories as $key=>$category)
                    <tr>
                        ...
                        <th scope="row">{{ $k }}</th>
                        <td >{{ $category->name }}</td>
                            <?php $v = new Verta($category->created_at); ?>
                        ...
                        <td>
                            <div class="row">
                                <div class="col-12" style="margin-bottom: 3px;">
                                    <a href="{{ route('category.edit', $category->id) }}?page={{request()->get('page',1)}}&s_status={{request()->get('s_status')}}&s_user_id={{request()->get('s_user_id')}}&s_name={{request()->get('s_name')}}" class="btn btn-info pull-right btn-width">ویرایش</a>
            ...
            {{ $categories->appends(['s_status' => request()->get('s_status')  , 's_name' => request()->get('s_name') , 's_user_id' => request()->get('s_user_id')])->links() }}
        </div>

        <a href="{{ route('category.create') }}?page={{request()->get('page',1)}}&s_status={{request()->get('s_status')}}&s_user_id={{request()->get('s_user_id')}}&s_name={{request()->get('s_name')}}" class="btn btn-success">موضوع جدید</a>
