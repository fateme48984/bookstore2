tinymce.PluginManager.add('cpaste', function(editor, url) {
    tinymce.PluginManager.requireLangPack('cpaste', '../../langs/fa_IR');

    editor.ui.registry.addIcon('cpaste', '<?xml version="1.0" encoding="UTF-8"?>\n' +
        '<svg width="24px" height="24px" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">\n' +
        '    <defs></defs>\n' +
        '    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">\n' +
        '        <path d="M18,9 L18,5 L16,5 L16,6 C16,6.55228475 15.5522847,7 15,7 L9,7 C8.44771525,7 8,6.55228475 8,6 L8,5 L6,5 L6,18 L9,18 L9,9 L18,9 Z M9,20 L6,20 C4.9,20 4,19.1 4,18 L4,5 C4,3.9 4.9,3 6,3 L9.18,3 C9.6,1.84 10.7,1 12,1 C13.3,1 14.4,1.84 14.82,3 L18,3 C19.1,3 20,3.9 20,5 L20,9 L21,9 L21,21 L9,21 L9,20 Z M10.5,10.5 L10.5,19.5 L19.5,19.5 L19.5,10.5 L10.5,10.5 Z M12,3 C11.45,3 11,3.45 11,4 C11,4.55 11.45,5 12,5 C12.55,5 13,4.55 13,4 C13,3.45 12.55,3 12,3 Z" fill="#000000" fill-rule="nonzero"></path>\n' +
        '    </g>\n' +
        '</svg>');

    var openDialog = function () {
        var cid = editor.getParam('content_id');
        var ctype = editor.getParam('content_type');
        var module_code = editor.getParam('module_code');
        var query = '?cid='+cid+'&ctype='+ctype+'&moduleCode='+module_code;
        return editor.windowManager.openUrl({
            url: tinymce.baseURL +'/plugins/cpaste/scripts/paste.php'+query,
            title: 'Paste',
            width: 740,
            height: 410,
        });
    };

    editor.ui.registry.addButton('cpaste', {
        tooltip: 'Paste',
        icon: 'cpaste',
        onAction: function () {
            // Open window
            openDialog();
        }

    });

    // Adds a menu item, which can then be included in any menu via the menu/menubar configuration
    editor.ui.registry.addMenuItem('cpaste', {
        text: 'Paste',
        icon: 'cpaste',
        onAction: function () {
            // Open window
            openDialog();
        }
    });
});
